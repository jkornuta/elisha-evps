  ...
  while ( wait )
  {
    if ( Serial.available() > 0 )
    {
      ...
      { 
        ...
        {
          // Wait to receive float (4 bytes)
          serialWaitBytes(4);
          input_data[i] = readFloatFromBytes();
        }
        ...
        //for ( uint32_t i = 0; i < data_length; i++ )
        //{
        //  Serial.println( input_data[i],DEC );
        //}
      ...
      // Did user type "f" for float test?
      if ( in == 'f' )
      {
        // Read in 4-byte float
        serialWaitBytes(4);
        float test_float = readFloatFromBytes();
        
        // Print out float for verification
        Serial.println( test_float, DEC );
      }
